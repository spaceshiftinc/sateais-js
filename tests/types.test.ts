...
/** submitDetection が呼ばれていないことを検証できる Fake */
...
describe("検証: ship / oilslick（scene_id か polygon+date）", () => {
  it("scene_id も polygon+date も無いと ValidationError", () => {
    ...
    expect(() => client.ship.detect({})).toThrow(ValidationError);
    expect(fake.submitDetection).not.toHaveBeenCalled();
  });

  it("polygon はあるが date が無いと ValidationError", () => {
    const { client } = makeClient();
    // @ts-expect-error date 欠落
    expect(() => client.ship.detect({ polygon: "POLYGON((0 0))" })).toThrow(
      ValidationError,
    );
  });
  ...
  it("scene_id が空文字なら ValidationError", () => {
    const { client } = makeClient();
    expect(() => client.oilslick.detect({ scene_id: "" })).toThrow(
      ValidationError,
    );
  });
...
describe("型: public API の判別・戻り値型", () => {
  it("DetectionEndpoint は 5 種の文字列リテラル", () => {
    expectTypeOf<DetectionEndpoint>().toEqualTypeOf<
